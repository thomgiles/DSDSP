...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(id = "tabs",
                menuItem("Q1: Data Tools (Input)", tabName = "q1_input", icon = icon("keyboard")),
                menuItem("Q1: Data Tools (Results)", tabName = "q1_results", icon = icon("chart-bar")),
                menuItem("Q2: Research Tasks (Input)", tabName = "q2_input", icon = icon("keyboard")),
                menuItem("Q2: Research Tasks (Results)", tabName = "q2_results", icon = icon("chart-bar")),
                menuItem("Q3: Research Aims (Input)", tabName = "q3_input", icon = icon("keyboard")),
                menuItem("Q3: Research Aims (Results)", tabName = "q3_results", icon = icon("chart-bar")),
                menuItem("Q4: Digital Security (Input)", tabName = "q4_input", icon = icon("keyboard")),
                menuItem("Q4: Digital Security (Results)", tabName = "q4_results", icon = icon("chart-bar"))
    )
  ),
  ...
  dashboardBody(
    ...
    tabItems(
      # =====================================================
      # Q1
      tabItem(tabName = "q1_input",
              fluidPage(
                titlePanel("Q1: What basic data tools do you need to do research at UoN?"),
                h5("Enter tools or platforms as a comma-separated list (phrases allowed)."),
                textAreaInput("q1_text", "Your tools:", width = "100%", rows = 3,
                              placeholder = "e.g. Excel, Python, Research Data Management Portal"),
                actionBttn("q1_submit", "Submit", color = "success", style = "fill")
              )
      ),
      tabItem(tabName = "q1_results",
              fluidPage(
                titlePanel("Q1 Responses: Data Tools"),
                h4("Live Wordcloud (refreshes every 3 s)"),
      ...
      # =====================================================
      # Q2
      tabItem(tabName = "q2_input",
              fluidPage(
                titlePanel("Q2: Match Research Tasks to Digital Tools"),
      ...
      tabItem(tabName = "q2_results",
              fluidPage(
                titlePanel("Q2 Responses: Research Task Mapping"),
                h4("Refreshes every 3 s"),
                tabsetPanel(
      ...
      # =====================================================
      # Q3
      tabItem(tabName = "q3_input",
              fluidPage(
                titlePanel("Q3: What digital tools may you need to support your research aims?"),
                pickerInput("q3_select","Select one or more tools:",
                            choices=c("R","Python","SQL","Excel","Teams","SharePoint",
                                      "GitHub","ORCID","RIS","UniCore","Other"),
                            multiple=TRUE,options=pickerOptions(actionsBox=TRUE)),
                actionBttn("q3_submit","Submit",color="success",style="fill")
              )
      ),
      tabItem(tabName = "q3_results",
              fluidPage(
                titlePanel("Q3 Responses: Research Tools Overview"),
      ...
      # =====================================================
      # Q4
      tabItem(tabName = "q4_input",
              fluidPage(
                titlePanel("Q4: Reflect on your current digital setup"),
      ...
      tabItem(tabName = "q4_results",
              fluidPage(
                titlePanel("Q4 Responses: Digital Security Overview"),
                h4("Average self-ratings across all participants (auto-refresh every 3 s)"),
...
server <- function(input, output, session) {
  ...
  render_task_plot <- function(task_name){
    renderPlotly({
      ...
      df <- responses$q2 %>% filter(Task==task_name) %>%
        group_by(Tool)%>%summarise(Freq=n(),.groups="drop")
  ...
  output$q4_radar <- renderPlotly({
    autoInvalidate()
    if(nrow(responses$q4)==0) return(NULL)

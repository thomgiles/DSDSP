...
<!-- Hidden by default -->
![](fig/contribute-digital-tools-qr.png){fig-align="center" width=400px}
...
- Interpret institutional on AI authorship, disclosure and data security  [UoN guidance](https://www.nottingham.ac.uk/studyingeffectively/studying/ai.aspx).
...
graph TD
    ...
    subgraph RNN["Recurrent Neural Network"]
        RNN_Idea["Sequential processing with hidden state"]
        RNN_Strength["Captures short-term structure"]
        RNN_Limit["Weak long-range memory"]
    end

    subgraph LSTM["Long Short-Term Memory Network"]
        LSTM_Idea["Gated memory cells"]
        LSTM_Strength["Retains long-term dependencies"]
        LSTM_Limit["Heavy to train"]
    end

    subgraph GRU["Gated Recurrent Unit"]
        GRU_Idea["Simplified LSTM"]
        GRU_Strength["Efficient, strong performance"]
        GRU_Limit["Still sequential"]
    end

    subgraph CNN["Convolutional Neural Network (for text)"]
        CNN_Idea["Local n-gram detection"]
        CNN_Strength["Parallel, fast"]
        CNN_Limit["Limited global context"]
    end

    %% Define styles
    classDef idea fill=#cce5ff,stroke=#004085,stroke-width=1px;
    classDef strength fill=#d4edda,stroke=#155724,stroke-width=1px;
    classDef limit fill=#f8d7da,stroke=#721c24,stroke-width=1px;

    %% Apply styles
    class RNN_Idea,LSTM_Idea,GRU_Idea,CNN_Idea idea;
    class RNN_Strength,LSTM_Strength,GRU_Strength,CNN_Strength strength;
    class RNN_Limit,LSTM_Limit,GRU_Limit,CNN_Limit limit;
```
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const base = "https://ds-shiny-helper.azurewebsites.net/edsex1/?embed=true";
...
flowchart TD
    ...
    D["✍️ Assist Writing & Visualisation"] 
...
Data analysis is often time‑consuming. AI assistants reduce friction by generating boilerplate code, spotting errors, and suggesting statistical methods or visualisations. 
...
Communicating results requires clarity and precision. AI tools can draft sections, polish language, and generate visual summaries. They help create abstracts, captions, and even graphical abstracts, allowing researchers to focus on interpretation.
...
The final stage is sharing findings. AI can adapt outputs for different audiences, from journal formatting to lay summaries and policy briefs. It helps amplify reach and accessibility without replacing researcher responsibility.
...
Generative models can produce outputs that sound fluent and convincing, yet contain factual errors or invented details. This risk is especially high when dealing with technical or niche topics where the model may lack reliable grounding.
...
Because models are trained on historical data, they inevitably absorb uneven representation and social biases. These can surface in analyses, summaries, or even in the framing of questions.
...
Generated text may unintentionally echo or replicate wording from training data. This raises questions about originality and intellectual property, particularly in academic contexts where citation and authorship are critical.

Paragraph example:  
AI‑generated drafts can resemble existing publications without clear attribution. To maintain integrity, researchers should apply transparent authorship practices, cite sources properly, and disclose AI involvement in writing.

> 📖 Proper citation and clear authorship are essential to avoid plagiarism and protect originality.

## Data protection and confidentiality
Submitting sensitive or unpublished information to external AI tools can expose researchers to institutional or legal risks. Confidential datasets, draft manuscripts, or proprietary results should not be processed without approval.
...
Over‑reliance on AI for interpretation, coding, or summarisation can gradually reduce researcher expertise. Skills such as critical reasoning, methodological design, and precise writing may weaken if delegated too often.
...
Researchers remain accountable for all outputs, regardless of AI involvement. Good practice includes verifying results, documenting AI contributions, and maintaining audit trails that show how tools were used.
...
Generative AI can accelerate research and stimulate new thinking when used responsibly. However, careless use can introduce errors, bias and ethical risks. Responsible integration requires caution, transparency and preservation of core analytical skills.
...
The opportunities and risks of generative AI lead naturally to a broader question: how can researchers use these tools responsibly? Academic research depends on accuracy, integrity and respect for participants and data. Generative AI does not remove these responsibilities; it changes how they must be upheld.

## Why ethics matters in AI-supported research

Generative AI influences how researchers read, write, analyse and interpret information. Because these systems produce fluent and confident output, errors can spread quickly if unchecked.
...
**Publisher and funder expectations** - Journals and funders increasingly require:

- clear statements describing AI use  
- confirmation that authors remain responsible for content  
- proof that citations and claims were manually verified  

## Good Practice for Researchers

| Principle | Summary | What Researchers Should Do |
|----------|---------|----------------------------|
| **1. Use AI for support, not substitution** | AI should enhance efficiency, not replace intellectual contributions. | Use AI for drafting structure, brainstorming or prototyping. Avoid delegating methodological decisions or accepting outputs without checking. |
| **2. Verify everything** | AI outputs may be incorrect or fabricated. | Check all claims against trusted sources, especially numerical results, statistical reasoning, literature summaries and citations. |
| **3. Maintain human oversight in analytical workflows** | AI-generated code and analyses require careful review. | Inspect intermediate outputs, validate assumptions, check data types and ensure outputs reflect the underlying evidence. |
| **4. Keep audit trails** | Documentation supports reproducibility and transparency. | Record tools, versions, prompts and validation steps. Note where AI influence begins and ends. |
| **5. Respect intellectual property** | AI may reproduce or echo training data. | Ensure originality, paraphrase appropriately and apply proper citation and academic judgement. |
| **6. Communicate transparently** | Openness about AI use maintains trust and integrity. | Disclose how AI was used in manuscripts, theses, presentations and collaborative projects. |

## Where ethical risks arise in research workflows

```mermaid
flowchart LR
    A[Input Data] --> B{Sensitive or Unpublished?}
    B -->|Yes| C[Do Not Submit to External AI]
    B -->|No| D[Proceed with Caution]

    D --> E[AI Processing]
    E --> F{Verified by Researcher?}
    F -->|No| G[Risk: Incorrect or Biased Output]
    F -->|Yes| H[Validated Output]

    H --> I[Incorporate into Research Workflow]
```
...
Generative AI introduces important considerations for how research data is handled, stored and protected. As AI becomes more integrated into research workflows, understanding data security and anticipating future developments is essential.
...
## How to protect research data when using AI
...
## Evolving institutional guidance

Universities are developing policies on:

- approved tools and infrastructure  
- disclosure requirements  
- acceptable use in assessment and authorship  
- skills training for staff and students  

This guidance will mature as AI becomes more embedded in research practice.

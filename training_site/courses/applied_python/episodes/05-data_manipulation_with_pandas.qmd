...
df.groupby("initial_severity")[day_cols].mean()
```

```{python}
summary = df.groupby("initial_severity")[day_cols].agg(["mean", "std", "count"])
print(summary)
```
...
files = sorted(Path().glob("data/inflammation-*.csv"))
if not files:
    raise SystemExit("No files matched 'data/inflammation-*.csv'")

...
```{python}
# Decide what the expected result should be before implementing the function.
# assert range_overlap([ (0.0, 1.0), (5.0, 6.0) ]) == <expected result>
```
...
```{python}
# Decide whether touching endpoints count as overlap in your context.
# assert range_overlap([ (0.0, 1.0), (1.0, 2.0) ]) == <expected result>
```
...
input_array = numpy.array([1.0, 2.0, 3.0])
average = numpy.mean(input_array)
...
This input should fail because the values cannot be converted safely to integers:

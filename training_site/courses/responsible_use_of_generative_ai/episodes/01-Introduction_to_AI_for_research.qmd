---
title: Introduction to AI for Research
metadata:
  teaching: 40
  exercises: 20
---
...
![Neural network flow showing input data passing through input and hidden layers to output data, with a feedback path for back-propagation.](fig/rai-neural-network-flow.svg){fig-align="center" width="78%"}

> 🧠 Learning improves when output errors are fed back into the model rather than treated as final answers.

> 🗣️ Language is challenging because meaning depends on context and relationships across sentences. 
...
At each step, the model attends over the tokens currently in its context window, using self-attention to weigh which parts of the prompt and prior output matter most for predicting the next token.

![Prompt generation loop showing a prompt entering an LLM, attention being computed, tokens emitted, and context updated.](fig/rai-context-generation-loop.svg){fig-align="center" width="78%"}

> 🧭 Context is active memory for the current exchange, not a guarantee of understanding.
...
> ✍️ The key transformation is therefore epistemic rather than technical: researchers are increasingly positioned as *editors of reasoning* rather than sole producers of intermediate artefacts.
...
> ⏸️ Faster movement between stages makes deliberate review points more important, not less.

> ⏸️ This compression increases throughput but reduces natural pauses for critical reflection, peer discussion, and methodological reconsideration. Opportunities therefore emerge only when deliberate checkpoints are reintroduced.
...
> ⏱️ These benefits are particularly significant in environments characterised by limited time, high cognitive load, or interdisciplinary translation.
...
> ⚖️ The point is not to ban AI in difficult areas. It is to match the tool, safeguards and review burden to the stakes.
...
> 🎛️ Opportunity only scales with *control*. Speed without control does not create research value.
...
![AI governance accountability flow showing AI-assisted contribution feeding outputs, claims, methods, interpretation, and researcher accountability.](fig/rai-governance-accountability.svg){fig-align="center" width="80%"}

> 🧾 Accountability cannot be delegated to the tool that generated or shaped the material.
...
> 👤 Data transparency is not an extra requirement created by AI; it should be an extension of normal research practice.
...
> 👤 The critical question is not whether AI was used, but how it was used
...
> ⚠️ Failing to disclose material AI contribution where required may seem minor, but can have significant ethical and governance implication. 
...
***“Data will be analysed using to identify informative variables.***

***Variables meeting a predefined relevance threshold will be retained. 

***Cases showing minimal contribution will be excluded and the the top 25 highest-ranked variables will be taken forward for interpretation and reporting.”***
...
> 🔐 The boundary crossing is the critical moment: once material leaves governed systems, controls become uncertain.
...
> 🔍 Drift is dangerous because each individual step can feel too small to question.
...
If not, it should not be shared with an external AI service. Check with your organisation directly if there are local / secure versions of these tools available for you to use.
...
> 🛡️ Use secure, approved tools; avoid sensitive data sharing; follow institutional, funder and publisher policies.
:::

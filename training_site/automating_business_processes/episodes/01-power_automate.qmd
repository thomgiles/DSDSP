...
![Power Automate sits inside the Microsoft Power Platform alongside Power Apps, Power BI, and Power Pages.](fig/power-platform-overview.svg){fig-align="center" width="88%"}

> 🧩 Power Automate is one part of the Microsoft Power Platform.

## Main Parts of a Flow

![A flow moves from trigger, to actions, to decision, to record, to notification.](fig/flow-anatomy.svg){fig-align="center" width="88%"}

> 🔁 A flow moves from trigger, to actions, to decision, to record, to notification.
...
3. Whenhere will the outcome be written?
...
![A trigger controls when the flow runs, what data is available, which connection is used, how often it can run, and what might fail.](fig/trigger-anatomy.svg){fig-align="center" width="88%"}

> 🚦 A trigger decides how the flow starts.
...
![Power Automate can connect to Microsoft 365, data stores, documents, communication tools, business apps, and APIs.](fig/connector-landscape.svg){fig-align="center" width="88%"}

> 🔌 Connector types determine which systems the flow can read from and write to.
...
| And a lot more! | HTTP, custom connectors, webhooks, external services |
...
![](fig/expression-pipeline.svg){fig-align="center" width="88%"}

> 🧮 Expressions shape values between steps.
...
> For example, `Compose nomalise request type` is clearer than `Compose 2`.
...
> Each branch should have a clear purpose and a clear end point.

## Branching with Conditions

![A condition checks a normalised value and routes work down urgent or standard paths.](fig/condition-branch.svg){fig-align="center" width="88%"}

> 🔀 Conditions route work down different paths.
...
![An approval pauses the flow, sends an approval card, waits for approve or reject, and then continues down the matching path.](fig/approval-overview.svg){fig-align="center" width="88%"}

> ✅ An approval pauses the flow for a human decision.
...
![A condition checks whether the approval outcome equals Approve, then routes to approved or rejected handling.](fig/approval-branch.svg){fig-align="center" width="88%"}
...
5. Add a **Condition** based on the approval outcome.
6. In the approved branch, send a confirmation email to yourself.
7. In the rejected branch, send a rejection email to yourself.
8. Save, test, complete the approval, and inspect run history.
...
Many business processes start as a spreadsheet.

Power Automate can turn each row into an action:
...
![A safe mailout reads rows, checks status, sends only ready rows, and updates sent rows.](fig/excel-mailout-pattern.svg){fig-align="center" width="88%"}

> 📧 A safe Excel-to-email pattern reads controlled rows, sends only ready messages, and updates the tracker.
...
## Update Rows After Sending

![After sending, update the row status and sent time so future runs skip already processed rows.](fig/update-rows-after-sending.svg){fig-align="center" width="88%"}

> 🧾 Update the row after the action.

## Use Word Templates

![Excel row values populate content controls in a Word template, then the generated document is saved and sent or stored.](fig/word-template-flow.svg){fig-align="center" width="88%"}

> 📄 Combine Excel rows with Word templates.
...
![The final workshop flow starts with an instant flow, reads Table1, applies each row to a Word template, sends an email with the generated attachment, and checks run history.](fig/workshop-build.svg){fig-align="center" width="88%"}

> 🏁 The final practice flow combines Excel rows, Word templates, test emails, and run history checks.

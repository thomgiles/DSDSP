...
1. Not scalable for long lists.
2. Hard to maintain when output format changes.
3. Fragile when list length changes.
...
This is concise and robust for longer lists:
...
In each cycle, `num` points to the next value in `odds` and the loop body runs once.
...
> 🧱 Correct loop syntax needs both `:` and indentation; missing either raises an error.
...
> 🏷️ Choose loop names that reflect the values they hold, not arbitrary placeholders.
...
> ✍️ Meaningful names make traces and debugging much easier for learners.
...
> 🔁 Confirm each loop iteration does exactly one predictable step.

## How `range` Parameters Work
...
> 🔁 Use loops to apply one operation consistently across many values.
...
## Worked Solution: Understanding Loops
...
## Worked Solution: Computing Powers With Loops
...
## Reflect: Computing Powers With Loops
...
## Worked Solution: Summing a List
...
## Worked Solution: Computing the Value of a Polynomial
...
> ⚖️ Comparison expressions return booleans used directly by `if` statements.
...
> 🔀 Test both true and false branches with small examples.

## Code Example: How Many Paths?
...
## Worked Solution: How Many Paths?
...
## Code Example: That's Not Not What I Meant
...
> 📏 Compare absolute difference to a tolerance instead of exact equality.
...
> 🧮 `abs` gives distance from zero, which makes tolerance checks easier.

## Hint Debrief: Close Enough

## Solution 1: Absolute Difference Check
...
> 🧠 This explicit branch style is useful when teaching conditional logic.

## Debrief: Solution 1

## Solution 2: Direct Boolean Print
...
> 🧹 Printing the boolean directly avoids duplicated `if`/`else` output code.
...
> ⚙️ In-place operators update and reassign in one concise statement.
...
## Reflect on Readability

## Worked Solution: In-Place Operators
...
> 🗂️ Classify each filename once and append it to exactly one output list.
...
## Starter Template: Sorting a List Into Buckets

Use the following Python code as your starting point:

## Starter Code: Sorting a List Into Buckets
...
## Success Criteria: Sorting a List Into Buckets
...
## Expected Final Lists: Sorting a List Into Buckets
...
## Worked Solution: Sorting a List Into Buckets
...
## Debrief: Sorting a List Into Buckets
...
> 🔤 Decide on case handling first, then keep your vowel rule consistent.

## Worked Solution: Counting Vowels

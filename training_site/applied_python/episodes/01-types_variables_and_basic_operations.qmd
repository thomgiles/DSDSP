...
> 🧪 Run this example and try changing the values in parentheses.
...
> 🧪 Run this example and try changing the arithmetic formula.
...
> 🧪 Run this example and try changing the value of `weight_kg`.
...
> 💡 Assignments link names to objects.
...
## Trace Variable Assignment Step by Step
...
> ✅ Match the number of variables on the left to the number of values on the right.

## Inspecting Types

Use `type()` when you are unsure what kind of value a variable holds. These can be used to investigate subtle bugs in arithmetic, comparisons, or string operations.
...
> ⚠️ Both variables changed because they refer to the same list in memory.
...
> ✅ `list(existing_list)` is a clear way to avoid accidental bugs from shared references.
...
> ⚠️ This flexibility is useful, but if not intentional, can make debugging harder.
...
- What about `element[4:]`? Or `element[:]`?
- What is `element[-1]`?
- What is `element[-2]`?
...
## Solution: Slicing strings from the end
...
Select the last 4 rows of the list. Aim for a solution that still works if the list length changes.
...
## Solution: Slicing lists from the end
...
Keep the same `step` but set the `start` index later to select a different pattern.
